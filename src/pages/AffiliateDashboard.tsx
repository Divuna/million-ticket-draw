  ...
  return (
    <div className="flex gap-1 rounded-xl border border-[hsl(var(--border)/0.5)] p-1 bg-black/30 w-fit" data-testid="mode-switcher">
      {items.map(({ id, label, Icon }) => {
        ...
        return (
          <button key={id} onClick={() => has && onSwitch(id)} disabled={!has}
            data-testid={`mode-btn-${id}`}
            title={!has ? `${label} — tento režim není součástí vašeho účtu. Kontaktujte administrátora.` : undefined}
          ...
          >
            ...
            {label}
            {!has && <span className="text-[10px] opacity-60 font-normal ml-0.5">– není aktivní</span>}
          </button>
        );
      })}
    </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 pt-2">
              <div className="rounded-xl border border-[hsl(var(--border)/0.4)] bg-white p-3 shadow-sm">
                <QRCodeSVG value={activeMode === 'influencer' ? customerLink : companyLink} size={140} data-testid="affiliate-qr-code" />
              </div>
