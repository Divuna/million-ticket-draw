    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          bonus_prize:bonus_prizes(id, description, image_url, title)
        `)
        .eq('user_id', user.id)
          ...
          <div className="space-y-4">
            {wins.map((win) => (
              ...
              >
                ...
                <CardContent>
                  {(() => {
                    // Determine the correct image source based on win type
                    const imageUrl = win.type === 'bonus' 
                    ...
                    return imageUrl ? (
                      <div className="mb-3 rounded-lg overflow-hidden">
                        <img 
                          src={imageUrl} 
                          alt={win.type === 'bonus' ? (win.bonus_prize?.title || 'Bonusová výhra') : win.contest?.title || 'Výhra'}
