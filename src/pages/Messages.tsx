...
export default function MessagesPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (inserted) {
        // Replace optimistic message with the real inserted row
        setMessages((prev) =>
          capAfterTailGrowth(
            prev.length,
        ...
        // Fetch only the AI reply generated by DB trigger (single targeted read)
        ...
        if (aiRows && aiRows.length > 0) {
          const aiMsg = aiRows[0] as Message;
          setMessages((prev) => {
            if (prev.some((m) => m.id === aiMsg.id)) return prev;
            return capAfterTailGrowth(prev.length, [...prev, aiMsg]);
          });
        }
      } else {
        setMessages((prev) =>
