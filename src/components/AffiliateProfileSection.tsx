...
const inputCls = "w-full bg-[hsl(var(--muted)/0.35)] border border-[hsl(var(--border)/0.6)] rounded-xl px-3.5 py-2.5 text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--ring)/0.4)] focus:border-[hsl(var(--ring))] transition-colors";
const labelCls = "text-[11px] uppercase tracking-wider text-[hsl(var(--text-muted-gray))] flex items-center gap-1.5 mb-1.5";
const readonlyCls = "rounded-xl border border-[hsl(var(--border)/0.4)] bg-[hsl(var(--muted)/0.2)] px-3.5 py-2.5";

/** Rotating accent hues for the profile's grouped section cards — same
 * language as the dashboard's SectionIcon, kept local since this component
 * doesn't share a module with AffiliateDashboard.tsx. */
const SECTION_HUES = ['262 80% 62%', '217 91% 60%', '152 55% 42%', '32 95% 54%'] as const;

function SectionCard({
  icon: Icon, hue = 0, title, description, children,
}: { icon: React.ElementType; hue?: number; title: string; description?: string; children: React.ReactNode }) {
  const color = SECTION_HUES[hue % SECTION_HUES.length];
  return (
    <div className="luxury-card p-6 space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {description && <p className="text-xs text-[hsl(var(--text-muted-gray))] mt-0.5">{description}</p>}
        </div>
      </div>
      {children}
    </div>
    ...
    <div className="space-y-5">
      ...
      <div className="luxury-card p-6 flex items-center gap-4">
        <div className="relative group">
          <Avatar className="w-16 h-16 border-2 border-[hsl(var(--border)/0.5)]">
        ...
        <div>
          <h3 className="text-lg font-bold text-[hsl(var(--foreground))]">Profil a výplatní údaje</h3>
          <p className="text-xs text-[hsl(var(--text-muted-gray))]">Spravujte své osobní údaje, fakturační adresu a platební informace. Klikněte na foto pro změnu.</p>
        </div>
      </div>
      ...
      <div className={`flex items-center gap-3 rounded-2xl border px-5 py-3.5 ${
        isPayoutReady
          ? 'border-[hsl(160_55%_45%/0.3)] bg-[hsl(160_55%_45%/0.07)]'
          : 'border-[hsl(43_90%_50%/0.3)] bg-[hsl(43_90%_50%/0.07)]'
      }`}>
        {isPayoutReady ? (
          <><CheckCircle2 className="w-4.5 h-4.5 text-[hsl(160_55%_40%)] shrink-0" /><span className="text-sm text-[hsl(160_55%_32%)]">Připraveno k výplatě — jakmile admin schválí provizi, bude odeslána na váš účet</span></>
        ) : (
          <><AlertTriangle className="w-4.5 h-4.5 text-[hsl(35_90%_42%)] shrink-0" /><span className="text-sm text-[hsl(35_90%_32%)]">Doplňte číslo účtu / IBAN pro příjem výplat (min. 8 znaků)</span></>
        )}
      </div>

      {/* Účet — read-only souhrn (nelze měnit) */}
      <SectionCard icon={ShieldCheck} hue={0} title="Účet" description="Nastavuje se při registraci a schválení — zde jen ke čtení.">
      ...
      {/* Kontaktní údaje */}
      <SectionCard icon={User} hue={1} title="Kontaktní údaje">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <SectionCard icon={FileDigit} hue={2} title="IČO / DIČ a plátcovství DPH">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* DPH */}
        <div className="mt-1 flex items-center gap-3 rounded-2xl border border-[hsl(var(--affiliate-purple)/0.2)] bg-[hsl(var(--affiliate-purple)/0.06)] px-4 py-3.5">
          <Switch
            checked={form.is_vat_payer}
            onCheckedChange={v => set('is_vat_payer', v)}
            id="vat-switch"
          />
          <label htmlFor="vat-switch" className="text-sm text-[hsl(var(--foreground))] cursor-pointer select-none">
            Jsem plátce DPH
      ...
      {/* Fakturační adresa */}
      <SectionCard icon={Building} hue={1} title="Fakturační adresa">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <SectionCard icon={Landmark} hue={3} title="Platební údaje" description="Na tento účet vám budeme posílat schválené výplaty provizí.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-[hsl(var(--border)/0.3)] bg-[hsl(var(--muted)/0.2)] px-4 py-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-[hsl(var(--foreground))]">CZK</p>
            </div>
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard icon={Instagram} hue={0} title="Sociální sítě a dosah" description="Zadejte odkazy na své profily jako text. Pomohou nám posoudit váš dosah.">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="flex justify-end">
        <Button
          ...
          disabled={saving}
          size="lg"
          className="gap-2 rounded-xl text-white font-semibold shadow-sm border-0"
