...
export const TicketResultModal: React.FC<TicketResultModalProps> = ({
    ...
    const fetchBonusPrize = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('id, title, description, detailed_description, image_url, amount')
  ...
  // Generate a local preview. Public sharing uses only the fixed-content OG
  // renderer and an opaque id; customer-generated bytes never leave the device.
  useEffect(() => {
  ...
  // A displayed bonus result is already awarded. Global lifecycle state stays
  // internal because its transitions reveal hidden draw progress.
  const isBonusClaimed = bonusPrize !== null;
        ...
        <div className="border-t border-transparent pt-4 mt-2" style={{ borderImage: 'linear-gradient(to right, transparent, rgba(255,138,0,0.4), transparent) 1' }}>
          ...
          <div className="flex justify-center gap-3">
            ...
            <Button
              ...
              disabled={isGeneratingImage || !publicShareUrl}
