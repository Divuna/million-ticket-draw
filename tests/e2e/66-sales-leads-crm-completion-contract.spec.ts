...
test.describe('Sales leads CRM completion contracts',()=>{
 ...
 test('public replyTo invariant is the trusted sales mailbox',()=>{expect(read('supabase/functions/send-sales-lead-reply/index.ts')).toMatch(/\breplyTo\b/);expect(read('supabase/functions/send-sales-lead-email/index.ts')).toMatch(/reply_to\s*:\s*REPLY_TO/);});
