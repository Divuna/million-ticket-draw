...
test.describe('Sales leads CRM completion contracts',()=>{
 ...
 test('replyTo invariant remains unchanged',()=>{expect(read('supabase/functions/send-sales-lead-reply/index.ts')).toContain('replyTo,');expect(read('supabase/functions/send-sales-lead-email/index.ts')).toContain('reply_to: replyTo');});
