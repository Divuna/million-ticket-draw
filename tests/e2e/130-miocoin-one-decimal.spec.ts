...
/**
 * Spec 130 — the MioCoin one-decimal rule (partner reward chain).
 *
 ...
 *
 * Scope: the PARTNER reward chain only (compute_partner_reward → issuance →
 * activation → invoicing). The Stripe top-up / refund / referral-from-payment path
 * is a separate reward system and is explicitly out of scope for this change — see
 * onemil_state.md for the boundary. That is why there is no `stripeWebhook` or
 * `paymentCredit` fixture here, unlike the historical draft of this spec.
 *
...
const columns = read('supabase/migrations/20260818100000_miocoin_one_decimal_columns.sql');
...
const dropLegacy = read('supabase/migrations/20260818100300_drop_legacy_partner_coin_bypass.sql');
...
test.describe('130 — automatic calculation rounds once, to one decimal', () => {
  ...
  test('660 Kč at 100 Kč = 3,7 MC yields 24,4 MC (the reported case)', () => {
    const raw = (660 / 100) * 3.7; // 24.42
    expect(raw).toBeCloseTo(24.42, 10);
    expect(roundMioCoin(raw)).toBe(24.4);
  });

  test('350 Kč at 100 Kč = 3,7 MC yields 13,0 MC', () => {
    const raw = (350 / 100) * 3.7; // 12.95
    expect(raw).toBeCloseTo(12.95, 10);
    expect(roundMioCoin(raw)).toBe(13);
  });
