...
-- ============================================================================
-- 2. Restore RPC/function bodies from captured production baseline.
--
-- Source of truth: docs/rollback/phase1_baseline.sql, Section B.
-- These CREATE OR REPLACE FUNCTION blocks restore full captured baseline bodies
-- instead of doing a text replacement against whatever body is live after apply.
-- ============================================================================
...
-- The baseline notes a second live 7-arg overload of admin_manage_bonus_prize,
-- but its full body was not captured in phase1_baseline.sql. The forward apply
-- updates every overload by function name, so this guarded fallback reverts only
-- that uncaptured overload if it exists. All captured functions above are restored
-- from full baseline CREATE OR REPLACE FUNCTION definitions.
DO $$
DECLARE
  r record;
  v_def text;
...
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'admin_manage_bonus_prize'
      AND pg_get_function_identity_arguments(p.oid) = 'p_prize_id uuid, p_contest_id uuid, p_description text, p_ticket_position integer, p_amount numeric, p_status text, p_operation text'
  LOOP
    v_def := pg_get_functiondef(r.oid);
    v_new := replace(v_def, 'role = ''superadmin''', 'role IN (''admin'', ''superadmin'')');

    IF v_new = v_def THEN
      RAISE EXCEPTION 'Phase 1 rollback refused: expected superadmin-only gate not found in uncaptured admin_manage_bonus_prize overload %.%', 'admin_manage_bonus_prize', r.args;
    END IF;
