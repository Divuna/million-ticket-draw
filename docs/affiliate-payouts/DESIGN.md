...
- **✅ TEST payout flow E2E na produkci (12. 06. 2026, schvaleni Pavla, app neverejna):** TEST provize `eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee` (1,23 Kc) → schvalena → doklad **APD-2026-000001** (PDF v privatnim bucketu) → davka **APB-2026-000005** → `.kpc` export (`exported`). E-mail `divispavel2@gmail.com` dorucen s PDF prilohou ✅; `influencer@onemil.c` rizene failed (neplatna adresa) ✅. Zadna platba, nic paid, `dddddddd-…` nedotcen. Cleanup proveden (provize, doklad, davka, items, email_queue smazany). 2 orphan soubory v privatnich bucketech (`affiliate-payout-docs/2026/eeeeeeee-…/APD-2026-000001.pdf`, `affiliate-bank-exports/2026/APB-2026-000005.kpc`) — neskodne, rucne smazatelne ve Storage (SQL delete blokuje protection trigger). Ponechano zamerne: affiliate `cd74ff3a` `payout_account=12545857` + `payout_bank=0800` (RPC validace: ucet bez kodu banky, 4ciferny kod zvlast); Botanic TEST data — **nahradit realnymi udaji pred verejnym spustenim**. Pozn.: `affiliate_payout_documents.email_status` zustava `pending` i po odeslani — worker nesynchronizuje stav zpet na doklad (kosmeticka audit mezera).
