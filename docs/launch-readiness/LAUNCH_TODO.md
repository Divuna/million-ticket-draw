...
| ID | Prio | Oblast | Krok | Očekávaný výsledek | Skutečný | Odkaz | Důkaz | Stav | Pozn. |
|----|------|--------|------|--------------------|----------|-------|-------|------|-------|
| SEC01 | P0 | Security advisor backlog | Projít a uzavřít/akceptovat nálezy Security Advisoru | Každý nález fixnut nebo výslovně akceptován ownerem | **PRODUKCE: Group 1 (advisor 23→10) + Group 2 safe/interim (advisor 10→8) hotové (schválení Pavla). STAGING navíc: E09 `admin_winner_delivery_stats` → `security_invoker=on` ověřeno (advisor 8→7, E2E `27512219000` 122 passed, output 786/297 nezměněn). E05/E23 security_invoker NELZE (tickets RLS = deny-all → vynulovalo by admin totaly). Produkce pro E09 NEDOTČENA.** | [SECURITY_FINDINGS.md](./SECURITY_FINDINGS.md) | staging advisor 8→7 + E2E 27512219000 | **selhalo (blocker)** | **P0 blocker: produkce 8 ERROR; E09 jen staging (čeká prod schválení); E05/E23 potřebují tickets admin-read policy nebo accept; Group 3 (E17–E20/E22); WARN/INFO** |
