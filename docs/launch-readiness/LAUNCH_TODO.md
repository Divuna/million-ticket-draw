...
| P04 | P1 | Přepočet MioCoinů | Nastavit reward_base/mc | Uloženo, ovlivní výpočet | spec 56b: toast „Nastavení odměn bylo uloženo" se ZOBRAZÍ, ale DB zůstane reward_base_czk=0/reward_mc=0. **REÁLNÁ RLS CHYBA:** `public.partners` nemá žádnou UPDATE policy (staging i produkce) → partner UPDATE vrátí 0 řádků + null error; `PartnerDashboard.tsx:857` nekontroluje počet změněných řádků → falešný success. | /partner/dashboard | GH run 27573182299 (failed) | **FAILING — RLS blocker** | spec 56b fixme; vyžaduje schválení Pavla: UPDATE policy na partners (partner-own) + app check affected rows |
