...
| L04 | P0 | Cookies | Owner/legal review policy proti reálným nástrojům + banneru | Policy text odpovídá reálným cookies/storage/tracking chování | **Technický mismatch cookie banneru opraven 15.06.: kanonická cookies stránka je owner-managed CMS `/legal/cookies` (`content_pages section=legal slug=cookies`) přes `/admin/content`; cookie banner odkazy míří na `/legal/cookies`. Meta `<noscript>` tracking image fallback zůstává odstraněn z `index.html`; `consent.ts` beze změny, JS Meta Pixel flow zůstává jen po marketing consentu. Právní text/CMS beze změny.** | banner, /legal/cookies | code link update + noscript/consent check 15.06. | **selhalo (blocker)** | L04 zůstává P0 jen do owner/legal potvrzení finálního cookies textu `/legal/cookies` |
