...
| ID | Prio | Oblast | Krok | Očekávaný výsledek | Skutečný | Odkaz | Důkaz | Stav | Pozn. |
|----|------|--------|------|--------------------|----------|-------|-------|------|-------|
| SEC01 | P0 | Security advisor backlog | Projít a uzavřít/akceptovat nálezy Security Advisoru | Každý nález fixnut nebo výslovně akceptován ownerem | **PRODUKCE: raw advisor 2 ERROR, ale **E22 `contest_progress` FORMÁLNĚ OWNER-ACCEPTED (Pavel, 15.06.2026)** jako záměrný veřejný agregát (není blocker). **Efektivní nevyřešený SEC01 ERROR = 1: E17** (`v_influencer_referrals_paid`, affiliate-scoped RLS redesign — NO-GO naslepo). E18 vyřešen na prod (advisor 3→2). Progrese prod: 23→…→2 (z toho 1 accepted).** | [SECURITY_FINDINGS.md](./SECURITY_FINDINGS.md) | owner accept E22 + prod advisor | **selhalo (blocker)** | **P0 blocker zůstává kvůli E17. E22 accepted (ne blocker). Po vyřešení E17 lze SEC01 uzavřít (mimo WARN/INFO).** |
