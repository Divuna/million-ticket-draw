...
| L04 | P0 | Cookies | Owner/legal review policy proti reálným nástrojům + banneru | Policy text odpovídá reálným cookies/storage/tracking chování | **Čistý audit 15.06. z aktuálního `origin/main` (`2eb29291166bea4685d8f11184e999766403fb06`, čistý detached checkout) nahrazuje předchozí audit z `codex/affiliate-payouts-audit`. Produkční CMS `/legal/cookies` existuje + is_active, `content_length=2328`, obsahuje `podpora@onemil.cz`; L09 e-mail mismatch vyřešen (`info@`/`support@` v aktivním CMS 0×). L04 ale zůstává P0: owner/legal musí potvrdit text proti realitě: Supabase Auth `localStorage.onemil-auth`, `localStorage.cookie_consent`, `public.cookie_consents`, GA4, GTM, Meta Pixel, Meta noscript fallback, OneSignal SDK/worker/cache/IndexedDB/`user_devices`, Stripe checkout redirect a app `localStorage`/`sessionStorage` klíče.** | banner, /privacy, /legal/cookies | clean origin/main audit + prod CMS read-only 15.06. | **selhalo (blocker)** | P0 dokud Pavel/legal nepotvrdí aktualizovaný cookies text; technický follow-up: prověřit Meta noscript fallback |
