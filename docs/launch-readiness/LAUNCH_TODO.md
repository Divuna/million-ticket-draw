...
| ID | Prio | Oblast | Krok | Očekávaný výsledek | Skutečný | Odkaz | Důkaz | Stav | Pozn. |
|----|------|--------|------|--------------------|----------|-------|-------|------|-------|
| L01 | P0 | Obchodní podmínky | Owner/legal review obsahu | Naplněno, aktuální | **Export CMS 14.06.: `/vop` existuje + is_active, ale obsah VELMI KRÁTKÝ → nedostatečný, nutný owner/legal review.** | /terms, /vop | export CMS 14.06. | **selhalo (blocker)** | P0 blocker: VOP obsah příliš stručný |
| L02 | P0 | Pravidla soutěží | Owner/legal review obsahu | Tikety 1,2,3…, pozice předem, ne loterie, bez placeholderů | **Export CMS 14.06.: `/pravidla-souteze` existuje + is_active, ale obsahuje PLACEHOLDERY `[NÁZEV SOUTĚŽE]`, `[DATUM]`, `[POPIS HLAVNÍ VÝHRY]`, `[HODNOTA]` → nesmí do veřejného spuštění.** | /pravidla-souteze | export CMS 14.06. | **selhalo (blocker)** | P0 blocker: placeholdery v pravidlech |
| L03 | P0 | GDPR/Privacy | Sjednotit + ověřit obsah | Jedna konzistentní verze | **Export CMS 14.06.: `/gdpr` i `/legal/ochrana-osobnich-udaju` OBA existují, ale WORDING SE LIŠÍ → nutno sjednotit a ověřit.** | /gdpr, /legal/ochrana-osobnich-udaju, /privacy | export CMS 14.06. | **selhalo (blocker)** | P0 blocker: duplicitní/nekonzistentní GDPR text |
| L04 | P0 | Cookies | Ověřit policy vs reálné nástroje + banner | Policy text odpovídá reálným cookies + chování banneru | **Export CMS 14.06.: `/legal/cookies` existuje + is_active (content_length≈2325); banner `CookieConsentBanner` zapisuje `cookie_consents`. Nutno OVĚŘIT policy proti reálným cookie nástrojům a chování banneru.** | banner, /privacy, /legal/cookies | export CMS 14.06. | **selhalo (blocker)** | P0 blocker: cookie policy vs realita |
| L05 | P0 | Kontakt | Reálné údaje | Dle COMPANY_CONTEXT.md | Owner potvrzeno: kanonický veřejný support e-mail je `podpora@onemil.cz`; cleanup hotový, žádné live/source-of-truth použití staré support adresy nezůstává | /kontakt, /terms, /privacy, /delete-account, footer | owner decision + cleanup audit 14.06. | prošlo | Public support e-mail consistency resolved; stará support adresa zůstává jen ve starých audit/history notes |
...
| L09 | P0 | E-maily v právních CMS textech | Owner/legal potvrzení kontaktních e-mailů | Sjednocené, správné kontaktní e-maily v právních textech | **Export CMS 14.06.: některé právní CMS texty stále obsahují `info@onemil.cz`; veřejný support source of truth je `podpora@onemil.cz` (L05). Které e-maily kam patří v právních textech musí potvrdit owner/legal PŘED editací.** | /vop, /gdpr, /legal/* | export CMS 14.06. | **selhalo (blocker)** | NEpřepisovat právní texty bez owner/legal potvrzení; L05 řešil jen veřejný support display |
...
| ID | Prio | Oblast | Krok | Očekávaný výsledek | Skutečný | Odkaz | Důkaz | Stav | Pozn. |
|----|------|--------|------|--------------------|----------|-------|-------|------|-------|
| SEC01 | P0 | Security advisor backlog | Projít a uzavřít/akceptovat nálezy Security Advisoru | Každý nález fixnut nebo výslovně akceptován ownerem | **PRODUKCE (schválení Pavla): Group 1 (23→10) + Group 2 safe/interim (10→8) + E09 (8→7) + E05/E23 (7→5) hotové a ověřené. E05/E23: `tickets_admin_select_all` policy + security_invoker (migrace `sec01_e05_e23_tickets_admin_read_and_invoker`); prod advisor 7→5; P0 smoke `27525944645` success; output 0/127/4000 nezměněn; customer own-row beze změny. Zbývá 5 ERROR = pouze Group 3.** | [SECURITY_FINDINGS.md](./SECURITY_FINDINGS.md) | prod advisor 7→5 + smoke 27525944645 | **selhalo (blocker)** | **P0 blocker: produkce 5 ERROR = Group 3 (E17 v_influencer_referrals_paid, E18 partner_api_activity, E19 contest_miocoin_totals, E20 winners_with_contest, E22 contest_progress) + WARN/INFO** |
