...
>
> ### ✅ UPDATE 15.06.2026 — SEC01 EFFECTIVELY RESOLVED (all ERROR fixed or owner-accepted)
> E17 affiliate-scoped redesign applied to **PRODUCTION** (`xkzhjldrojjlrkezorey`, owner-approved; migration `sec01_e17_influencer_referrals_paid_affiliate_scoped`) after precheck matched baseline. Postcheck: view `security_invoker=on`, anon=false, authenticated=true, count preserved (0=0), `influencer_referrals_owner_admin` active, helpers anon-exec=false/auth-exec=true. Production advisor **ERROR 2 → 1**; P0 smoke `27529591097` = success, 5 passed. No rollback needed.
> **Production ERROR progression: 23 → 10 → 8 → 7 → 5 → 3 → 2 → 1.** The single remaining raw ERROR is **E22 `contest_progress`, which is formally owner-accepted** (intentional public aggregate). → **All SEC01 ERROR findings are now fixed or owner-accepted; SEC01 is no longer a launch blocker.** (WARN/INFO backlog remains separate, non-blocking.)
>
> ### UPDATE 15.06.2026 — E17 affiliate-scoped redesign APPLIED + VERIFIED on STAGING (`dxmowysntemfqfnanxua`) only
> Migration `sec01_e17_influencer_referrals_paid_affiliate_scoped`: (1) replaced `influencer_referrals` broad `USING(true)` with `influencer_referrals_owner_admin` (affiliate-own via `partners.auth_user_id=auth.uid()` + admin); (2) added minimal-disclosure SECURITY DEFINER helpers `user_completed_first_topup(uuid)` + `referral_user_is_valid(uuid)` (anon exec=false, authenticated=true) so no raw payments/`auth.users` are exposed; (3) rebuilt `v_influencer_referrals_paid` as `security_invoker=on` over `influencer_referrals` only. Postcheck: invoker on, anon=false, authenticated=true, count preserved (0=0). Staging advisor **E17 gone (ERROR 2 → 1)**; remaining 1 = E22 (already owner-accepted) → **effective unresolved staging ERROR = 0**. Full Staging E2E `27528853194` = success, **122 passed, 0 failures** (affiliate dashboard + admin influencers work). **Production NOT touched** — E17 ready for separate production approval. After prod E17 rollout, SEC01 can be closed (E22 accepted).
>
> ### UPDATE 15.06.2026 — E22 `contest_progress` FORMALLY OWNER-ACCEPTED (Pavel)
> Documentation only — no SQL, no advisor-count change. Production raw advisor still reports 2 ERROR, but **E22 is now an accepted intentional public aggregate**, so the **effective unresolved SEC01 ERROR count = 1 (E17 only)**.
> Owner accept (Pavel, 15.06.2026): *„Jako vlastník OneMil formálně akceptuji bezpečnostní nález Security Advisoru `Security Definer View` na `public.contest_progress` jako záměrné veřejné agregované údaje: počet prodaných tiketů, počet zbývajících tiketů a procento naplnění soutěže. View neobsahuje osobní ani citlivá data. Ponechává se `SECURITY DEFINER`, protože `security_invoker=on` by rozbil veřejné zobrazení průběhu soutěže — zákazník by viděl jen své tikety, ne celkový stav soutěže. Tento nález se nepovažuje za blocker."*
>
> ### UPDATE 15.06.2026 — E18 partner-own RLS redesign APPLIED + VERIFIED on PRODUCTION (`xkzhjldrojjlrkezorey`, owner-approved)
> Same migration `sec01_e18_partner_api_activity_partner_own_rls` applied to production after precheck matched baseline (RLS on, 0 policies, invoker off; 5 partners with auth_user_id; 6 real `partner_api_requests` rows). Postcheck: `partner_api_requests_partner_own` present, `security_invoker=on`, anon=false, authenticated=true. Production advisor **ERROR 3 → 2** (E18 gone). Production P0 smoke `27528174542` = success, 5 passed. **No rollback needed.** Remaining production ERROR = **E17** (`v_influencer_referrals_paid`, redesign) + **E22** (`contest_progress`, owner-accept). SEC01 stays **P0 blocker**.
>
> ### (staging) E18 partner-own RLS redesign APPLIED + VERIFIED on STAGING (`dxmowysntemfqfnanxua`)
> Migration `sec01_e18_partner_api_activity_partner_own_rls`: added `partner_api_requests_partner_own` SELECT policy (partner-own via `partners.auth_user_id=auth.uid()` + admin) on `partner_api_requests`, then `security_invoker=on` on `partner_api_activity`. Postcheck: policy present, invoker on, anon=false, authenticated=true. Staging advisor **ERROR 3 → 2** (E18 gone). Full Staging E2E `27527383016` = success, **122 passed, 0 failures** (spec 47 partner dashboard 47e/47f green). **Production NOT touched** — E18 ready for separate production approval. Remaining staging ERROR: E17 (redesign) + E22 (owner-accept).
>
> ### (prior) Group 3 safe/interim APPLIED + VERIFIED on PRODUCTION (`xkzhjldrojjlrkezorey`, owner-approved)
> Same migration `sec01_group3_safe_interim_hardening` applied to production after precheck matched baseline (E17–E20 anon already false / auth true / invoker off; contest_progress untouched). Postcheck: E19 + E20 anon=f/auth=f/invoker=on; E17/E18 anon=f/auth=t (unchanged — anon was already revoked on prod, so the REVOKE was a no-op; they remain interim); E22 unchanged. Production advisor **ERROR 5 → 3** (E19+E20 gone; remaining = E17, E18, E22). Production P0 smoke `27526912855` = success, 5 passed. **No rollback needed.** SEC01 stays **P0 blocker**. (Rollback caveat: the standard rollback would GRANT anon back to E17/E18, which would over-open them vs prod baseline — for prod, rollback of E17/E18 = no action.)
>
> ### (staging) Group 3 safe/interim APPLIED + VERIFIED on STAGING (`dxmowysntemfqfnanxua`)
> Migration `sec01_group3_safe_interim_hardening`: E19 `contest_miocoin_totals` + E20 `winners_with_contest` (both unused) → revoked anon/auth + `security_invoker=on` (**cleared**); E18 `partner_api_activity` + E17 `v_influencer_referrals_paid` → **anon revoked (interim; SDV ERROR remains, redesign pending)**; E22 `contest_progress` **unchanged** (owner-accept candidate). Postcheck: E19/E20 anon=f/auth=f/invoker=on; E17/E18 anon=f/auth=t; E22 untouched. Staging advisor **ERROR 5 → 3** (remaining = E17, E18, E22). Full Staging E2E `27526273831` = success, **122 passed, 0 failures** → Games/ContestDetail, partner dashboard, affiliate dashboard all work. **Production NOT touched.** SEC01 stays **P0 blocker**. Path to 0 ERROR needs: E18 partner-own RLS redesign, E17 affiliate-scoped RLS redesign, E22 formal owner accept (all owner decisions).
>
> ### (prior) E05/E23 RESOLVED on PRODUCTION (`xkzhjldrojjlrkezorey`, owner-approved)
> Same migration `sec01_e05_e23_tickets_admin_read_and_invoker` applied to production after precheck matched baseline (tickets 2 own-row policies, no admin policy, invoker off). Postcheck: `tickets_admin_select_all` present (tickets now 3 policies), E05+E23 `security_invoker=on`, output unchanged (activity 0 rows [no tickets in last 24h], revenue 127 rows, tickets_sold 4000). Production advisor **ERROR 7 → 5** (E05+E23 gone; remaining 5 = Group 3 only). Production P0 smoke `27525944645` = success, 5 passed. **No rollback needed.** Customer own-row reads unchanged; admin policy gated by `has_role` → no privacy impact. SEC01 stays **P0 blocker** (5 ERROR = Group 3).
>
> ### (earlier) E05/E23 RESOLVED on STAGING via tickets admin-read policy + `security_invoker`
...
| E05 | ERROR | Security Definer View | `public.contest_activity_last_24h` | fixed (production, verified) | `security_invoker=on` + `tickets_admin_select_all` policy (prod, migration `sec01_e05_e23_tickets_admin_read_and_invoker`); prod advisor 7→5; smoke 27525944645; output unchanged | Done — admin reads via tickets admin policy; customers keep own-row |
...
| E17 | ERROR | Security Definer View | `public.v_influencer_referrals_paid` | fixed (production, verified) | affiliate-scoped redesign (prod, migration `sec01_e17_influencer_referrals_paid_affiliate_scoped`): `influencer_referrals` owner+admin RLS, 2 minimal-disclosure SECURITY DEFINER helpers, view rebuilt `security_invoker=on` over base table only; prod advisor 2→1; smoke 27529591097; count preserved | Done — affiliate sees only own; admin all; no raw payments/auth.users exposed |
| E18 | ERROR | Security Definer View | `public.partner_api_activity` | fixed (production, verified) | partner-own RLS `partner_api_requests_partner_own` + `security_invoker=on` (prod, migration `sec01_e18_partner_api_activity_partner_own_rls`); prod advisor 3→2; smoke 27528174542 | Done — partner sees only own API activity (6 prod rows scoped), admin all |
| E19 | ERROR | Security Definer View | `public.contest_miocoin_totals` | fixed (production, verified) | `security_invoker=on` + revoked anon/auth (prod, migration `sec01_group3_safe_interim_hardening`); prod advisor 5→3; smoke 27526912855 | Done (unused aggregate; bonus_prizes public-readable) |
| E20 | ERROR | Security Definer View | `public.winners_with_contest` | fixed (production, verified) | `security_invoker=on` + revoked anon/auth (prod, migration `sec01_group3_safe_interim_hardening`); prod advisor 5→3; smoke 27526912855 | Done (unused; winners already public-readable) |
...
| E22 | ERROR | Security Definer View | `public.contest_progress` | **accepted-risk (owner: Pavel, 15.06.2026)** | formal owner accept (see banner) | Intentional public aggregate (tickets sold/remaining, % filled); **no private data**. `security_invoker=on` would BREAK it (customers read only own tickets → wrong counts). Left as SECURITY DEFINER by design; **not a launch blocker** |
| E23 | ERROR | Security Definer View | `public.contest_revenue` | fixed (production, verified) | `security_invoker=on` + `tickets_admin_select_all` policy (prod, migration `sec01_e05_e23_tickets_admin_read_and_invoker`); prod advisor 7→5; smoke 27525944645; output unchanged | Done — admin reads via tickets admin policy; customers keep own-row |
