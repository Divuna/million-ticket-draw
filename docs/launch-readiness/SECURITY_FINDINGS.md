...
| E17 | ERROR | Security Definer View | `public.v_influencer_referrals_paid` | fixed (staging, verified) / prod pending | affiliate-scoped redesign (staging, migration `sec01_e17_influencer_referrals_paid_affiliate_scoped`): `influencer_referrals` owner+admin RLS, 2 minimal-disclosure SECURITY DEFINER helpers, view rebuilt `security_invoker=on` over base table only; staging advisor E17 gone (2→1); E2E 27528853194 | Done on staging — affiliate sees only own; admin all; no raw payments/auth.users exposed. Prod apply pending owner approval |
