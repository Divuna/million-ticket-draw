...
| E22 | ERROR | Security Definer View | `public.contest_progress` | owner-accept candidate (intentional public aggregate) | analysis 14–15.06 | Public contest progress (counts/% sold), no private data. `security_invoker` would BREAK it (customers read only own tickets → wrong counts). Read by customer pages incl. anon. Recommend formal owner accept; left unchanged |
