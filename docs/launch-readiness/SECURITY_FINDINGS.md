...
>
> ### UPDATE 15.06.2026 — E18 partner-own RLS redesign APPLIED + VERIFIED on STAGING (`dxmowysntemfqfnanxua`) only
...
| E18 | ERROR | Security Definer View | `public.partner_api_activity` | fixed (staging, verified) / prod pending | partner-own RLS `partner_api_requests_partner_own` + `security_invoker=on` (staging, migration `sec01_e18_partner_api_activity_partner_own_rls`); staging advisor 3→2; E2E 27527383016 (spec 47 green) | Done on staging — partner sees only own API activity, admin all. Prod apply pending owner approval |
