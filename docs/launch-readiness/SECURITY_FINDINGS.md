...
>
> ### UPDATE 15.06.2026 — Group 3 safe/interim APPLIED + VERIFIED on PRODUCTION (`xkzhjldrojjlrkezorey`, owner-approved)
...
| E18 | ERROR | Security Definer View | `public.partner_api_activity` | interim (production): anon already revoked; SDV ERROR remains; redesign pending | prod baseline anon=false; migration `sec01_group3_safe_interim_hardening` (no-op for anon on prod) | MEDIUM risk (cross-partner API metadata). Full fix = partner-own RLS on `partner_api_requests` + `security_invoker` (owner decision); invoker alone would empty PartnerDashboard (base table deny-all) |
