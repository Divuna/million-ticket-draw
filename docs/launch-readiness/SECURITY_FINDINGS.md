...
>
> ### UPDATE 15.06.2026 — Group 3 safe/interim APPLIED + VERIFIED on STAGING (`dxmowysntemfqfnanxua`) only
...
| E17 | ERROR | Security Definer View | `public.v_influencer_referrals_paid` | interim (staging): anon revoked; SDV ERROR remains; redesign pending | migration `sec01_group3_safe_interim_hardening`; E2E 27526273831 | HIGH risk (cross-affiliate user data). anon revoked (staging). Full fix = affiliate-scoped RLS on base tables + grants (owner decision); `security_invoker` alone would break (underlying Group-1 views revoked) |
| E18 | ERROR | Security Definer View | `public.partner_api_activity` | interim (staging): anon revoked; SDV ERROR remains; redesign pending | migration `sec01_group3_safe_interim_hardening`; E2E 27526273831 | MEDIUM risk (cross-partner API metadata). anon revoked (staging). Full fix = partner-own RLS on `partner_api_requests` + `security_invoker` (owner decision); invoker alone would empty PartnerDashboard (base table deny-all) |
| E19 | ERROR | Security Definer View | `public.contest_miocoin_totals` | fixed (staging, verified) / prod pending | `security_invoker=on` + revoked anon/auth (staging, migration `sec01_group3_safe_interim_hardening`); staging advisor 5→3; E2E 27526273831 | Done on staging (unused aggregate; bonus_prizes public-readable). Prod apply pending owner approval |
| E20 | ERROR | Security Definer View | `public.winners_with_contest` | fixed (staging, verified) / prod pending | `security_invoker=on` + revoked anon/auth (staging, migration `sec01_group3_safe_interim_hardening`); staging advisor 5→3; E2E 27526273831 | Done on staging (unused; winners already public-readable). Prod apply pending owner approval |
