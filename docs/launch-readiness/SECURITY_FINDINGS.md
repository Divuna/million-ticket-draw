...
| E17 | ERROR | Security Definer View | `public.v_influencer_referrals_paid` | interim (production): anon already revoked; SDV ERROR remains; redesign pending | prod baseline anon=false; migration `sec01_group3_safe_interim_hardening` (no-op for anon on prod) | HIGH risk (cross-affiliate user data). Full fix = affiliate-scoped RLS on base tables + grants (owner decision); `security_invoker` alone would break (underlying Group-1 views revoked) |
