# Zásady ochrany osobních údajů

**Platforma OneMil** — www.onemil.cz

Verze: 2.0 (návrh) | Zpracováno: 7. 9. 2026

---

## 1. Kdo je správcem vašich údajů

Správcem osobních údajů je společnost:

**iCONIC POINT s.r.o.**
IČO: 17795851
DIČ: CZ17795851
Sídlo: Na Folimance 2155/15, Vinohrady, 120 00 Praha 2
Zapsaná v obchodním rejstříku vedeném Městským soudem v Praze, oddíl C, vložka 376856

**Kontakt ve věcech ochrany osobních údajů:**
E-mail: podpora@onemil.cz
Datová schránka: c9mizui
Adresa: Na Folimance 2155/15, Vinohrady, 120 00 Praha 2

Správce nejmenoval pověřence pro ochranu osobních údajů. Ve všech záležitostech týkajících se
zpracování vašich údajů se proto obracejte na výše uvedené kontakty.

Tyto zásady vysvětlují, jaké údaje o vás zpracováváme, proč je potřebujeme, komu je předáváme
a jaká máte práva. Postupujeme podle Nařízení Evropského parlamentu a Rady (EU) 2016/679 (GDPR).

---

## 2. Jaké údaje zpracováváme

### Údaje, které nám poskytnete

| Kategorie | Konkrétně |
|---|---|
| Identifikační údaje | jméno a příjmení |
| Kontaktní údaje | e-mailová adresa, telefonní číslo |
| Doručovací údaje | adresa pro předání věcné výhry (ulice, město, PSČ, země) |
| Potvrzení věku | potvrzení, že je vám alespoň 18 let |
| Přihlašovací údaje | e-mail a heslo, případně identifikátor účtu Google nebo Facebook, pokud se přihlašujete přes ně |
| Obsah komunikace | zprávy, které nám napíšete na zákaznickou podporu |

### Údaje, které vznikají používáním platformy

| Kategorie | Konkrétně |
|---|---|
| Údaje o účtu | datum registrace, stav účtu, role, poslední aktivita |
| MioCoiny | zůstatek a historie připsání a odečtení |
| Tikety | soutěže, kterých jste se zúčastnili, a čísla vašich tiketů |
| Výhry | evidence hlavních a bonusových výher a stav jejich předání |
| Platby | částka, datum, stav platby a identifikátor transakce u platební brány |
| Odměny od partnerů | kódy MioCoinů získané nákupem u partnerských e-shopů a jejich uplatnění |
| Doporučení | zda vás pozval jiný uživatel a jaké odměny z toho vznikly |

### Technické údaje

| Kategorie | Konkrétně |
|---|---|
| Zařízení a prohlížeč | typ zařízení, prohlížeč, operační systém |
| Síťové údaje | IP adresa |
| Cookies a obdobné technologie | podrobnosti najdete v [Zásadách používání souborů cookies](https://onemil.cz/legal/cookies) |
| Souhlas s cookies | jaké kategorie cookies jste povolili a kdy |
| Registrace k oznámením | identifikátor zařízení pro zasílání upozornění, pokud je povolíte |

**Nezpracováváme** údaje o vaší platební kartě. Platbu provádíte přímo u platební brány Stripe;
my se dozvíme pouze to, že platba proběhla, její výši a identifikátor transakce.

**Zvláštní kategorie osobních údajů** — například údaje o zdravotním stavu, politických názorech
nebo náboženském přesvědčení — od vás pro používání OneMil **nevyžadujeme ani je cíleně
neshromažďujeme**. Nemáme na ně žádné pole a k ničemu je nepotřebujeme.

Pokud takový údaj sami uvedete ve volném textu zprávy zákaznické podpoře nebo asistentovi
zákaznické podpory, může se stát součástí této komunikace. Proto do zpráv neuvádějte citlivé údaje,
které pro vyřízení vašeho dotazu nejsou potřeba.

---

## 3. Proč údaje zpracováváme a na jakém právním základě

### 3.1 Plnění smlouvy

Bez těchto údajů vám nemůžeme službu poskytnout.

- vytvoření a správa uživatelského účtu
- evidence MioCoinů a jejich připisování či odečítání
- účast v soutěžích a přidělování tiketů
- evidence výher a jejich předání
- zpracování plateb za MioCoiny
- vyřizování vašich dotazů na zákaznické podpoře

### 3.2 Plnění zákonné povinnosti

- vedení účetnictví a uchovávání daňových dokladů
- plnění povinností vyplývajících z právních předpisů ve vztahu k výhrám

### 3.3 Oprávněný zájem

- zabezpečení platformy a ochrana před zneužitím, podvody a automatizovanými útoky
- ověření, že je vám alespoň 18 let
- zlepšování a rozvoj platformy
- určení, výkon nebo obhajoba právních nároků

Proti zpracování na základě oprávněného zájmu můžete kdykoli vznést námitku (viz článek 7).

### 3.4 Souhlas

- analytické a marketingové cookies a obdobné technologie
- zasílání obchodních sdělení
- zasílání upozornění do zařízení, pokud je povolíte

Souhlas je vždy dobrovolný a můžete jej kdykoli odvolat. Odvolání souhlasu nemá vliv na zákonnost
zpracování před jeho odvoláním a neovlivní vaši možnost platformu dál používat.

---

## 4. Komu údaje předáváme

Vaše údaje neprodáváme. Předáváme je pouze poskytovatelům, kteří pro nás zajišťují provoz
platformy, a to v rozsahu nezbytném pro danou službu.

| Služba | Role | Jaké údaje |
|---|---|---|
| **Supabase** | databáze, autentizace a serverová část platformy | veškeré údaje o účtu, MioCoinech, tiketech, výhrách, platbách a komunikaci |
| **Vercel** | hosting a doručování webové aplikace | technické údaje spojené s návštěvou webu |
| **Stripe** | platební brána | údaje nutné k provedení platby a jejímu spárování s vaším účtem |
| **Resend** | odesílání e-mailů | e-mailová adresa a obsah zprávy, kterou vám posíláme |
| **OneSignal** | zasílání upozornění do zařízení | identifikátor zařízení, pokud upozornění povolíte |
| **Google** | měření návštěvnosti a reklama | technické údaje a údaje o používání webu, pouze pokud povolíte příslušné cookies |
| **Meta** | reklama na Facebooku a Instagramu | technické údaje a údaje o používání webu, pouze pokud povolíte marketingové cookies |
| **Sofinity** | provozní vyhodnocování a zpracování zpráv zákaznické podpory | identifikátor uživatele a obsah zpráv podpory |
| **OpenAI** | jazykový model pro asistenta zákaznické podpory | údaje popsané v článku 8 |
| Dopravci | doručení věcné výhry | jméno, adresa a telefon, pouze pokud vyhrajete věcnou cenu |

Uvedena jsou obchodní označení jednotlivých služeb. Přesnou identifikaci právnické osoby, která
konkrétní službu provozuje, a její roli zpracovatele vám na vyžádání sdělíme na podpora@onemil.cz.

Údaje můžeme dále předat orgánům veřejné moci, pokud nám to ukládá zákon.

### Předávání mimo Evropskou unii

Někteří z výše uvedených poskytovatelů jsou usazeni ve Spojených státech amerických nebo mohou
údaje zpracovávat mimo Evropskou unii a Evropský hospodářský prostor.

Při takovém předání se opíráme o záruky, které příslušný poskytovatel nabízí podle kapitoly V
nařízení GDPR. Konkrétní záruku uplatňovanou u jednotlivého poskytovatele vám na vyžádání sdělíme
na podpora@onemil.cz.

---

## 5. Jak dlouho údaje uchováváme

| Kategorie | Doba uchování |
|---|---|
| Údaje o účtu a jeho obsahu | po dobu trvání registrace |
| Účetní a daňové doklady | po dobu stanovenou právními předpisy |
| Údaje potřebné k obhajobě právních nároků | po dobu trvání příslušné promlčecí lhůty |
| Souhlas s cookies | po dobu platnosti souhlasu a poté jako doklad o jeho udělení |
| Komunikace se zákaznickou podporou | po dobu trvání registrace |

Po zániku účtu údaje odstraníme nebo anonymizujeme, s výjimkou těch, které jsme povinni uchovávat
podle právních předpisů nebo které potřebujeme k obhajobě právních nároků.

**Zrušení účtu.** O zrušení účtu a výmaz údajů můžete požádat kdykoli. Postup, jak žádost podat,
najdete na stránce [Smazání účtu](https://onemil.cz/delete-account); žádost můžete poslat také
přímo na podpora@onemil.cz. Účet se neruší automaticky samotným otevřením té stránky — jde
o podání žádosti. Po jejím potvrzení vaše osobní údaje smažeme do 30 dnů, s výjimkou údajů, které
jsme povinni uchovávat podle právních předpisů nebo které potřebujeme k obhajobě právních nároků.

---

## 6. Zabezpečení

Přijali jsme technická a organizační opatření, která odpovídají povaze zpracovávaných údajů
a možným rizikům. Patří mezi ně zejména:

- šifrovaný přenos dat mezi vaším zařízením a platformou,
- řízení přístupu k databázi na úrovni jednotlivých záznamů,
- omezení přístupu k údajům pouze na pověřené osoby,
- ochrana proti automatizovaným útokům a zneužití účtu.

Žádné opatření nedokáže vyloučit riziko úplně. Pokud by došlo k porušení zabezpečení, které by
pravděpodobně znamenalo vysoké riziko pro vaše práva a svobody, budeme vás informovat.

---

## 7. Vaše práva

Ve vztahu ke svým osobním údajům máte tato práva:

| Právo | Co znamená |
|---|---|
| **Přístup** | dozvědět se, jaké údaje o vás zpracováváme, a získat jejich kopii |
| **Oprava** | nechat opravit nepřesné nebo doplnit neúplné údaje |
| **Výmaz** | nechat údaje smazat, pokud už nejsou potřebné nebo odvoláte souhlas |
| **Omezení zpracování** | dočasně omezit zpracování, například po dobu ověřování přesnosti údajů |
| **Přenositelnost** | získat údaje, které jste nám poskytli, ve strojově čitelném formátu — u zpracování na základě souhlasu nebo smlouvy |
| **Námitka** | vznést námitku proti zpracování na základě oprávněného zájmu |
| **Odvolání souhlasu** | kdykoli odvolat souhlas, který jste nám dali |

Své právo uplatníte na **podpora@onemil.cz**. Odpovíme vám nejpozději do jednoho měsíce od přijetí
žádosti. Pokud by byla žádost složitá, můžeme lhůtu prodloužit — v takovém případě vás o tom
vyrozumíme.

Základní údaje si můžete kdykoli sami zobrazit a upravit ve svém profilu v aplikaci.

**Právo podat stížnost:** Domníváte-li se, že zpracováním vašich údajů porušujeme právní předpisy,
můžete se obrátit na dozorový úřad:

Úřad pro ochranu osobních údajů
Pplk. Sochora 27, 170 00 Praha 7
www.uoou.cz

---

## 8. Asistent zákaznické podpory a automatizované zpracování

Součástí platformy je asistent zákaznické podpory využívající jazykový model poskytovatele OpenAI.
Asistent funguje pouze tehdy, když je tato funkce zapnutá.

**Co se asistentovi předává, když ho použijete:**

- obsah vaší aktuální zprávy,
- omezenou historii téže konverzace, nejvýše deset předchozích zpráv,
- **jméno z vašeho profilu**, aby vás mohl oslovit,
- **omezené údaje o vašem účtu potřebné k odpovědi** — zůstatek MioCoinů, počet výher, stav
  voucherů a čas vaší poslední zprávy v konverzaci,
- přehled aktuálně dostupných soutěží.

Tyto údaje o účtu předáváme proto, aby vám asistent dokázal odpovědět konkrétně, například kolik
máte MioCoinů, místo obecné odpovědi.

**Vaši e-mailovou adresu do konverzace nepřidáváme.** Pokud ji ale napíšete přímo do textu své
zprávy, stane se součástí zprávy a bude předána spolu s ní — stejně jako jakýkoli jiný údaj, který
do zprávy sami uvedete.

Asistenta můžete kdykoli obejít — v konverzaci požádejte o předání lidskému operátorovi. Pokud si
nepřejete, aby byl obsah vaší komunikace takto zpracován, napište nám přímo na podpora@onemil.cz.

**Nepoužíváme automatizované rozhodování**, které by pro vás mělo právní účinky nebo se vás
obdobně významně dotýkalo. O výhře v soutěži nerozhoduje žádný algoritmus vyhodnocující vaši osobu:
hlavní výhru získává držitel posledního tiketu a bonusové výhry jsou přiřazeny k předem určeným
pozicím tiketů.

---

## 9. Děti

Platforma OneMil je určena výhradně osobám starším 18 let. Vědomě nezpracováváme údaje osob
mladších 18 let. Pokud zjistíme, že jsme takové údaje získali, účet zrušíme a údaje odstraníme.

---

## 10. Změny těchto zásad

Tyto zásady můžeme čas od času aktualizovat, například když začneme používat novou službu nebo
se změní právní předpisy. Aktuální znění je vždy dostupné na www.onemil.cz. O podstatných změnách
vás budeme informovat e-mailem nebo upozorněním v aplikaci.
