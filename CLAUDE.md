Warning: truncated output (original token count: 74018)
...
- **Staging `dxmowysntemfqfnanxua`:** policy `apd_admin_all` na `public.affiliate_payout_documents` změněna z `is_admin()` (ALL, USING+WITH CHECK) na `public.is_superadmin()` (ALL, USING+WITH CHECK). **Změněna JEN tato jedna policy** (…24018 tokens truncated… odeslání e-mailu s dokladem · vytvoření dávky · generování Air Bank exportu · označení dávky zaplacené · odeslání potvrzení o zaplacení.
...
## OBCHOD / LEADY — ODPOVĚDI A DUPLICITNÍ KONTAKTY (10. 07. 2026, pouze PR)

- Stavové hodnoty zůstávají: `konvertovan` se zobrazuje „Spolupráce", `odmitl` „Bez spolupráce" a `nekontaktovat` zůstává samostatná blokace.
- Přesný kontaktní e-mail se kontroluje vždy. Doména kontaktního e-mailu se kontroluje jen u firemních domén; veřejné služby jsou v `sales_lead_public_email_domains`.
- Duplicitu smí potvrdit pouze držitel `sales_leads.manage` (superadmin je zahrnut) s povinným důvodem. Výjimka musí být auditovaná v `sales_lead_duplicate_overrides` i `sales_lead_activities`.
- Každá odesílací Edge Function musí bezprostředně před Resendem volat service-role-only `sales_lead_email_send_guard`; frontend nikdy není autoritativní.
- Migrace `20260710180000_sales_leads_replies_duplicate_overrides.sql` není aplikovaná. `send-sales-lead-reply` není nasazená. Žádný e-mail nebyl odeslán.
