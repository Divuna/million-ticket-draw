...
- **Fáze 4B (UI navržených leadů + RPC editace zařazení) — UI v main, RPC jen na STAGINGU (04. 07. 2026, schválení Pavla):** PR #179 mergnutý do `main` (merge commit `d3c4fa3ff30aa4bd7c0152a7adf5fff674f1743d`); UI: záložka „Návrhy" (`navrzeny`), skupina `lead_group`, sekce „Zařazení" (edit `lead_group`/`lead_quality`/`discovery_source`, `discovery_meta` read-only), akce Schválit/Nekontaktovat/Archivovat přes `sales_lead_set_status`. Migrace `20260704130000_sales_leads_phase4b_discovery_update_rpc.sql` aplikována **pouze na staging `dxmowysntemfqfnanxua`** přes `apply_migration`, `{"success": true}`. RPC `sales_lead_update_discovery`: SECURITY DEFINER, anon bez EXECUTE, authenticated s EXECUTE, guard `sales_leads.manage`. Ověřeno na test leadu `c48f2567-…`: mění `lead_group`/`lead_quality`/`discovery_source`, nemění `discovery_meta`, nemění status, neodesílá e-mail. **Produkce tuto RPC migraci zatím nemá; Lovable Publish neproběhl.** Bez EF deploye, bez e-mailu; nedotčeny wallets/payments/contests/tickets/winners/Stripe/`buy_ticket_atomic`.
