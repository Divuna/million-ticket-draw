...
- **Fáze 5C (discover vyžaduje veřejný e-mail) — připraveno JEN jako soubory v PR (05. 07. 2026, neaplikováno/nenasazeno):** EF `sales-lead-discover` upravena — AI v jednom volání vrací i `email`/`email_source_url`/`email_confidence` per firma (stejná přísná pravidla jako `sales-lead-enrich-contact`, nikdy nevymýšlet). Firma **bez** platného veřejného e-mailu + zdroje se PŘESKOČÍ dřív, než se zavolá `sales_lead_propose` — žádný lead pro ni nevznikne, jen `outcome:"skipped", reason:"missing_public_email"` v odpovědi. Firma **s** e-mailem: lead vznikne (`navrzeny`) a e-mail se hned uloží jako neověřený návrh přes `sales_lead_propose_contact` (`proposed_contact_email`/`_source_url`, `status='neovereny'`); `contact_email`/`email_verified_by_admin` beze změny (null/false) — schvaluje jen člověk „Schválit e-mail". UI `DiscoverLeadsDialog.tsx`: text „Uloží se jen firmy s dohledaným veřejným e-mailem."; výsledek běhu ukazuje vytvořeno/celkem přeskočeno/z toho kvůli chybějícímu e-mailu. Žádná nová DB migrace. `npx tsc --noEmit` 0 chyb, `npm run build` ✅. **Nic nenasazeno; produkce i staging nedotčeny; žádný e-mail neodeslán; žádný produkční lead vytvořen; Lovable Publish neproběhl.**
