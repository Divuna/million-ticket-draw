...
- **Fáze 4A (DB základ navržených leadů) — aplikováno a ověřeno POUZE na stagingu (04. 07. 2026, schválení Pavla):** migrace `20260704120000_sales_leads_phase4a_proposed_leads.sql` na **staging `dxmowysntemfqfnanxua`** přes `apply_migration` (ne `db push`), `{"success": true}`. **Produkce nedotčena.** Staging: `status` povoluje `navrzeny`; sloupce `lead_group`/`lead_quality`/`discovery_source`/`discovery_meta`; CHECK kontroly + index `idx_sales_leads_lead_group`; `sales_lead_set_status` blokuje `navrzeny → schvaleni_ceka`, povoluje `navrzeny → novy`. Test lead `c48f2567-5daf-4ce0-ab6b-89192748eef8` (`STAGING ONLY`). Bez EF deploye, bez Lovable Publish, bez e-mailu. **Produkční aplikace Fáze 4A čeká na samostatné schválení Pavla.**
