...
e-mailu se nesmí ukládat jako lead. **Vše jen jako soubory v PR — nic nenasazeno. Produkce
`xkzhjldrojjlrkezorey` i staging `dxmowysntemfqfnanxua` nedotčeny. Žádný e-mail neodeslán.
Žádná produkční data vytvořena/smazána. Žádná nová DB migrace — plné reuse RPC z Fáze 5A/5B.**
...
- **EF `supabase/functions/sales-lead-discover/index.ts`:** AI v jednom volání vrací i
  ...
  e-mailu + zdroje (chybí/neplatný formát/`confidence:"low"`) se PŘESKOČÍ **před** voláním
  `sales_lead_propose` — žádný lead nevznikne; v odpovědi `outcome:"skipped",
  reason:"missing_public_email"`. Firma **s** e-mailem: nejdřív vznikne lead (`navrzeny`),
  poté se e-mail uloží jen jako neověřený návrh přes `sales_lead_propose_contact`
  (`proposed_contact_email`/`_source_url`, `status='neovereny'`); `contact_email` a
  `email_verified_by_admin` zůstávají beze změny (null/false) — schvaluje jen člověk.
...
- **Aplikace/deploy EF + Lovable Publish = samostatné kroky, vyžadují výslovné schválení Pavla.**
...
- **Migrace:** `supabase/migrations/20260629120000_partner_own_select_rls.sql` — 3 SELECT-only partner-own + admin/superadmin policies. Žádné write policy. Zápisy zůstávají přes SECURITY DEFINER RPC / service_role.
- **Staging ✅:** aplikováno na `dxmowysntemfqfnanxua`; postcheck OK — partner vidí jen vlastní (9 PRC / 2 PCA / 0 PAK), cross-partner = 0, admin vše (15/5/4), partner write blokován (UPDATE 0 rows, INSERT RLS-denied), BOHEMIA staging nezměněna.
- **Produkce ✅ (29. 06. 2026, schválení Pavla):** aplikováno na `xkzhjldrojjlrkezorey` (transakční COMMIT); postcheck OK — 3× SELECT policies, 0 write policies, BOHEMIA vidí 5 own PRC (2 Shoptet, 1 aktivovaný), admin vidí vše (5/3/17), data intaktní (4 issued, 1 activated, `auth_user_id` intaktní, `delivery='partner'` beze změny). Dashboard se partnera projeví po znovunačtení.
- **UI nezměněno** — `weeklyReports` v `PartnerDashboard.tsx` je správné; oprava je čistě DB RLS.
- **Rollback:** 3× `DROP POLICY IF EXISTS` (viz migrace).
...
- **DB migrace:** `20260628120000_shoptet_connection_requests.sql` — jediný soubor, atomická transakce. Obsahuje: `partners.reward_trigger_status` (default `'paid'`), `shoptet_connection_requests` tabulka (15 sloupců) + 4 RLS policies + 4 indexy + trigger, 3 Vault RPC (service_role only).
- **EF `submit-shoptet-connection` v3 ACTIVE** — partner podá žádost v `/partner/dashboard`, URL jen ve Vault (nikdy v DB).
- **EF `approve-shoptet-connection` v3 ACTIVE** — admin schválí/zamítne, approve: promote Vault URL, `shoptet_customer_delivery='onemil'` SET (non-negotiable), `reward_trigger_status` zkopírován, `shoptet_import_enabled=true`.
- **EF `import-shoptet-orders` v5 ACTIVE** — respektuje `reward_trigger_status` threshold, 5-bucket status taxonomie (completed/shipped/paid/cancelled/pending), idempotentní, batches log v 500-řádkových blocích.
- **Lovable Publish:** UI pro `/partner/dashboard` (Step 5 + formulář) a `/admin/partners` (Step 6 + badge) — ✅ dokončeno.

**Produkční postcheck ✅ (29. 06. 2026):**
- `shoptet_connection_requests` table + 4 RLS policies + 4 indexy: ✅
- Vault RPC (`store_`, `promote_`, `delete_`, `get_shoptet_export_url`): anon=false, service_role=true ✅
- BOHEMIA (`61c23960-7271-4c75-a1a4-dcb6e81b41ce`): `shoptet_customer_delivery='partner'` beze změny ✅
- 0 testovacích SCR řádků ✅
- Všechny 3 EF ACTIVE (submit v3, approve v3, import v5) ✅
- Žádné URL, customer emaily, reward kódy v DB (Vault only) ✅
...
- BOHEMIA má `delivery='partner'` — nikdy neprochází self-service flow.
- Shoptet URL se NIKDY neukládá do aplikační tabulky — výhradně Vault.
- `reward_trigger_status` default `'paid'` zachovává BOHEMIA legacy chování.

**Commit:** `c8d3f7bb` (CLAUDE.md). Zbývá Lovable Publish (✅ zvládnuto).
...
- Zákazník dostane kód e-mailem od OneMil ihned po vydání.
- Phase 2 implementační návrh připraven 28. 06. 2026 (nová tabulka `shoptet_connection_requests`, EF `submit-shoptet-connection` + `approve-shoptet-connection`, partner UI formulář, admin badge + schvalovací flow).
...
- OneMil vytvoří kód, ale doručení zákazníkovi zajistí partner vlastním systémem.
...
- **Test ✅** (existující reálný řádek + dočasný role flip v transakci, rollback): superadmin→1, admin/subadmin→0, normální uživatel→0, anon→0; admin přímý INSERT zablokován RLS WITH CHECK `42501`. Existující řádek beze změny (`total_rows=1`), role `admin:2`; policy ponechána.
...
- **Další objekt:** `affiliate_payout_batches` / `apb_admin_all`. Produkční krok: schválení Pavla + manuální `pg_dump` (PITR off).
...
- **DB stav migrace:** `supabase/migrations/20260622_admin_subadmins_overview.sql` **aplikováno na staging i produkci**. Produkce `xkzhjldrojjlrkezorey` ověřena (22. 06. 2026): `public.get_admin_subadmins_overview` existuje, SECURITY DEFINER, owner postgres, execute jen `authenticated`, `anon` blokován. Promote/demote/invite na produkci fungují; status-badge tabulka na produkci ožije po Lovable Publish frontendu.
