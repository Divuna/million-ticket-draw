-- MioCoin — one-decimal rule, part 1/3: column types and validation guards.
--
...
-- existing 2-decimal financial behaviour and are deliberately untouched.
--
-- Read-only production audit performed before writing this migration
-- (project xkzhjldrojjlrkezorey, 16. 08. 2026):
--   * partners.reward_mc                    12 rows, 0 with >1 decimal, 0 below 0.5
--   * partner_product_reward_rules.fixed_mc  1 row,  0 with >1 decimal, 0 below 0.5
--   * partner_product_reward_rules.ratio_mc  0 rows
--   * shoptet_connection_requests.reward_mc  1 row,  0 with >1 decimal
--   * partner_reward_codes.coins            13 rows, all integers (3..26)
--   * partner_coin_activations.coins         4 rows, all integers (3..10)
--   * partner_invoice_lines.coins            1 row,  integer
--   * partner_invoices.coins_activated       6 rows, all integers
--   * no view depends on any of these columns
--   → every constraint below is satisfied by current production data.
--
-- BLOCKING FINDING recorded during the same audit, deliberately NOT touched here:
--   wallets.balance_coins has 1 row with 2 real decimals (10117.91) and
--   wallet_transactions.amount has 1 row with 2 real decimals (999.99, top-up of
--   2026-03-16). Those come from the CZK top-up path, not from partner rewards.
--   No wallet data is migrated, rounded or constrained by this migration.
--
-- Rollback:
