-- ============================================================================
-- Magin lead supply adapter: read-only discovery job results
-- ============================================================================
-- Adds the smallest safe read path Magin needs after creating discovery jobs:
-- read only the leads created by those jobs, identify backend-verified proposed
-- leads, and return their ids for the existing approval adapter.
--
-- No discovery worker, lead creation, approval, batch, or email logic is changed.
...
BEGIN
  ...
  WITH scoped_leads AS (
    ...
    WHERE l.discovery_meta->>'job_id' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      ...
      AND l.lead_group = 'e-shopy'
      AND l.source = 'ai_vyhledavani'
  ),
  classified_leads AS (
    SELECT
      id,
...
COMMENT ON FUNCTION public.sales_lead_magin_get_discovery_job_results(uuid[], uuid) IS
  'Narrow service-role-only read adapter for Magin: returns e-shopy leads created by the supplied discovery jobs and eligible lead_ids for existing approval RPC.';
