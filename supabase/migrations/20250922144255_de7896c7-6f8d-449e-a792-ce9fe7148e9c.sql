-- Create CRUD test setup function using existing user
-- This approach works around the foreign key constraints
...
BEGIN
    ...
    DELETE FROM notifications WHERE user_id = v_user_id AND title LIKE '%CRUD Test%';
    GET DIAGNOSTICS v_cleanup_count = ROW_COUNT;
    
    DELETE FROM payments WHERE user_id = v_user_id AND amount = 999.99;
    
    DELETE FROM tickets WHERE user_id = v_user_id AND number = 77777;
    
    DELETE FROM vouchers WHERE user_id = v_user_id AND code LIKE 'CRUD-TEST-%';
    ...
    INSERT INTO wallets (user_id, balance_coins, balance_vouchers, created_at)
    ...
    SELECT json_build_object(
        ...
        'user_id', v_user_id,
        'user_email', (SELECT email FROM users WHERE id = v_user_id),
...
-- Create CRUD test validation function
...
BEGIN
    ...
    ELSE
        SELECT id INTO v_user_id FROM users WHERE role IN ('admin', 'superadmin') LIMIT 1;
    END IF;
    ...
    SELECT json_build_object(
        ...
        'test_data_validation', json_build_object(
            'wallet_exists', (SELECT count(*) FROM wallets WHERE user_id = v_user_id) > 0,
            'wallet_balances', (
                SELECT json_build_object('coins', balance_coins, 'vouchers', balance_vouchers)
                FROM wallets WHERE user_id = v_user_id
            ),
            'test_notifications', (SELECT count(*) FROM notifications WHERE user_id = v_user_id AND title LIKE '%CRUD Test%'),
            'test_payments', (SELECT count(*) FROM payments WHERE user_id = v_user_id AND amount = 999.99),
            'test_tickets', (SELECT count(*) FROM tickets WHERE user_id = v_user_id AND number = 77777),
            'test_vouchers', (SELECT count(*) FROM vouchers WHERE user_id = v_user_id AND code LIKE 'CRUD-TEST-%'),
            'audit_logs', (SELECT count(*) FROM audit_logs WHERE user_id = v_user_id AND event = 'crud_test_setup')
        )
    ) INTO v_results
    FROM users u 
...
GRANT EXECUTE ON FUNCTION setup_crud_test_data(text) TO authenticated;
GRANT EXECUTE ON FUNCTION validate_crud_test_data(text) TO authenticated;