-- ============================================================================
-- Fix buy_voucher_atomic to issue one unique voucher_code on every new purchase.
-- ============================================================================
-- Scope:
--   - Replaces only public.buy_voucher_atomic(p_user_id uuid, p_voucher_id uuid).
--   - Keeps voucher price, wallet debit, wallet transaction, and redeemed_count
--     trigger behavior unchanged.
--   - Does not backfill old user_vouchers without voucher_code_id.
--
-- Concurrency:
--   - Locks the wallet row.
--   - Locks the voucher row while checking availability.
--   - Locks exactly one available voucher_codes row with FOR UPDATE SKIP LOCKED.
--   - Links user_vouchers.voucher_code_id and voucher_codes.issued_* in the
--     same transaction, so the same code cannot be issued twice.
-- ============================================================================
