-- FÁZE 5 — osobní doporučení hráčů (23. 9. 2026)
--
-- Pravidla (potvrzena Pavlem):
...
--   * obě odměny jsou nepeněžní MIO s vlastní sadou (wallet lot) a 12měsíční
--     expirací od připsání; připisují se výhradně centrální lot cestou.
--
-- Opravuje:
--   * `create_referral_reward_from_payment` vytvářel `referral_rewards`, ale MIO
--     nikdy nepřipsal (a počítal z MIO včetně bonusu, ne z Kč),
--   * `reverse_referral_reward_on_payment_status_change` při stornu ODEČÍTAL
--     doporučujícímu MIO, která mu nikdy nebyla připsána,
--   * obnova po neúspěšné refundaci připisovala přes `try_credit_wallet_mc`
--     (mimo sady) i odměny, které nikdy připsané nebyly.
--
-- Reverze:
--   * refundace (prepare_stripe_refund) odečte 5% odměnu poměrně ke skutečně
--     refundovaným Kč; 15 MIO se odečte jen při refundaci CELÉ platby,
--   * odečítá se výhradně ze sady dané odměny; co už doporučující utratil, se
--     nevymáhá z jiných sad — zapíše se jako `reversal_shortfall_mc` + audit,
--   * neúspěšná Stripe refundace vrátí přesně odečtené pohyby do téže sady,
--   * 15 MIO je jedinečné na doporučeného uživatele (unikátní index) — refundace
--     prvního dobití nikdy neotevře možnost získat bonus podruhé.
--
...
alter table public.wallet_lots add constraint wallet_lots_source_check check (source in (
  ...
  'referral_reward', 'referral_first_topup_bonus'));
...
-- ===========================================================================
-- referral_rewards: typ odměny, vazba na sadu, evidence reverze
-- ===========================================================================
...
comment on column public.referral_rewards.lot_id is 'Sada MIO doporučujícího, do které byla odměna připsána. NULL = nikdy nepřipsáno (historické záznamy).';
comment on column public.referral_rewards.reversal_target_mc is 'Kolik MIO se mělo reverzí odečíst (poměr refundovaných Kč).';
comment on column public.referral_rewards.reversed_mc is 'Kolik MIO se reverzí skutečně odečetlo ze sady odměny.';
comment on column public.referral_rewards.reversal_shortfall_mc is 'Část reverze, kterou nešlo odečíst, protože doporučující MIO z této sady už utratil nebo expirovala.';
...
alter table public.referral_rewards add constraint referral_rewards_reversal_check check (
  ...
  and reversed_mc >= 0 and reversed_mc <= reversal_target_mc
  and reversal_shortfall_mc >= 0
...
-- ===========================================================================
-- Připsání odměny do sady doporučujícího (interní)
-- ===========================================================================
...
begin
  ...
  -- Idempotence: jednou připsaná odměna se nepřipisuje znovu.
  if v_rr.lot_id is not null or v_rr.credited_at is not null then
    return jsonb_build_object('credited', false, 'already_credited', true, 'lot_id', v_rr.lot_id);
  end if;
  ...
  -- Nepeněžní MIO → celé jako bonus_mio sady; expirace 12 měsíců od připsání.
  v_res := public.wallet_credit_lot(
    v_rr.referrer_user_id, v_rr.reward_mc, v_type,
    p_reward_id, 'referral_payment:' || coalesce(v_rr.payment_id::text, ''),
    null, null, 0, v_rr.reward_mc, null, v_type,
  ...
  insert into public.wallet_transactions (
    ...
    v_rr.referrer_user_id,
    (v_res->>'wallet_id')::uuid,
    v_rr.reward_mc,
    (v_res->>'new_balance')::numeric,
    v_type,
    'referral',
    v_rr.payment_id,
    jsonb_build_object('reward_id', p_reward_id,
  ...
  return jsonb_build_object('credited', true, 'lot_id', v_res->>'lot_id', 'amount', v_rr.reward_mc);
end;
$function$;
...
begin
  ...
  -- Jen dokončené PLACENÉ dobití se skutečně zaplacenými Kč (bonusové, partnerské,
  -- API ani historické platby bez Kč odměnu nezakládají).
  ...
  -- 15 MIO jen za PRVNÍ placené dobití doporučeného uživatele. Dřívější platba
  -- (dokončená, rozpracovaná refundace i refundovaná) nárok ruší; unikátní index
  -- navíc zaručí, že bonus vznikne nejvýše jednou i při souběhu.
  select not exists (
...
-- ===========================================================================
-- Reverze a obnova (interní)
-- ===========================================================================
create or replace function public._referral_reverse_for_payment(
  p_payment_id uuid, p_refund_fraction numeric, p_reason text)
 returns jsonb
 language plpgsql
 security definer
 set search_path to 'public'
...
declare
  r          record;
  v_frac     numeric := least(greatest(coalesce(p_refund_fraction, 0), 0), 1);
  v_target   numeric;
  v_avail    numeric;
  v_take     numeric;
  v_wallet   uuid;
...
begin
  for r in
    ...
    where payment_id = p_payment_id
      and status = 'earned'
      and reversal_target_mc = 0
    order by reward_type
    for update
  loop
    if r.reward_type = 'percent' then
      v_target := least(r.reward_mc, round(r.reward_mc * v_frac, 1));
    else
      -- 15 MIO se vrací jen při refundaci celé platby.
      v_target := case when v_frac >= 1 then r.reward_mc else 0 end;
    end if;
    ...
    if r.lot_id is not null then
      select id into v_wallet from public.wallets where user_id = r.referrer_user_id for update;
      perform public._wallet_lots_expire_user(r.referrer_user_id, v_wallet);
      ...
      if v_take > 0 then
        ...
        values (v_lot.id, r.referrer_user_id, 'referral_reversal', -v_take, p_reason, p_payment_id,
                jsonb_build_object('reward_id', r.id, 'reward_type', r.reward_type,
                                   'target', v_target, 'refund_fraction', v_frac));

        perform public._wallet_set_managed(true);
        update public.wallets
        set balance_coins = balance_coins - v_take
        where id = v_wallet
        ...
        insert into public.wallet_transactions (
          ...
          jsonb_build_object('reward_id', r.id, 'reward_type', r.reward_type,
                             'target', v_target, 'refund_fraction', v_frac, 'lot_id', v_lot.id)
        );
      end if;
    end if;

    -- Nikdy nepřipsaná historická odměna (lot_id NULL): jen stav, bez pohybu
    -- peněženky a bez evidované částky reverze.
    update public.referral_rewards
    set status                = case when v_target = r.reward_mc then 'reversed' else 'partially_reversed' end,
        reversed_at           = now(),
        reverse_reason        = p_reason,
        reversal_target_mc    = case when r.lot_id is null then 0 else v_target end,
        reversed_mc           = v_take,
        reversal_shortfall_mc = case when r.lot_id is null then 0 else v_target - v_take end
    where id = r.id;

    if r.lot_id is not null and v_target - v_take > 0 then
      insert into public.audit_logs (event, event_type, user_id, metadata, created_at)
      values ('referral_reversal_shortfall', 'referral_reward_integrity', r.referrer_user_id,
              jsonb_build_object('reward_id', r.id, 'payment_id', p_payment_id,
                                 'target', v_target, 'reversed', v_take,
                                 'shortfall', v_target - v_take, 'reason', p_reason), now());
      v_short := v_short + (v_target - v_take);
    end if;
  ...
  return jsonb_build_object('rewards', v_rows, 'reversed_mio', v_total, 'shortfall_mio', v_short,
                            'refund_fraction', v_frac);
end;
$function$;

create or replace function public._referral_restore_for_payment(p_payment_id uuid)
 returns jsonb
 language plpgsql
 security definer
 set search_path to 'public'
...
begin
  for r in
    ...
    where payment_id = p_payment_id
      ...
      and (reverse_reason = 'stripe_refund'
           or reverse_reason = 'payment_status_changed:refund_pending')
    for update
  loop
    if r.lot_id is not null and r.reversed_mc > 0 then
      select id into v_wallet from public.wallets where user_id = r.referrer_user_id for update;

      update public.wallet_lots
      set remaining_amount = remaining_amount + r.reversed_mc,
          status = case when status in ('reversed', 'depleted') then 'active' else status end,
          updated_at = now()
      where id = r.lot_id;

      insert into public.wallet_lot_movements (lot_id, user_id, movement_type, amount, reason, reference_id, metadata)
      values (r.lot_id, r.referrer_user_id, 'referral_restore', r.reversed_mc, 'stripe_refund_failed', p_payment_id,
              jsonb_build_object('reward_id', r.id, 'reward_type', r.reward_type));

      perform public._wallet_set_managed(true);
      update public.wallets
      set balance_coins = balance_coins + r.reversed_mc
      where id = v_wallet
      returning balance_coins into v_new;
      perform public._wallet_set_managed(false);
      ...
      insert into public.wallet_transactions (
        ...
        r.referrer_user_id, v_wallet, r.reversed_mc, v_new, 'referral_restore', 'reverse_failed_stripe_refund',
        p_payment_id, jsonb_build_object('reward_id', r.id, 'reward_type', r.reward_type, 'lot_id', r.lot_id)
      );

      -- Sada mezitím mohla expirovat: vrácená MIO pak řádně expirují s pohybem.
      perform public._wallet_lots_expire_user(r.referrer_user_id, v_wallet);

      v_total := v_total + r.reversed_mc;
    end if;
    ...
    set status                = 'earned',
        reversed_at           = null,
        reverse_reason        = null,
        reversal_target_mc    = 0,
        reversed_mc           = 0,
        reversal_shortfall_mc = 0
    where id = r.id;

    v_rows := v_rows + 1;
  end loop;

  return jsonb_build_object('rewards', v_rows, 'restored_mio', v_total);
end;
$function$;

-- Storno mimo refundační tok (platba přejde z `completed` do jiného stavu
-- jinak než přes prepare_stripe_refund, např. ruční změna na `refunded`/`failed`).
-- Refundace přes prepare_stripe_refund (`completed → refund_pending`) se tu
-- přeskakuje — reverzi dělá prepare poměrně k refundovaným Kč.
...
begin
  ...
  if old.status = 'completed'
    ...
    perform public._referral_reverse_for_payment(new.id, 1, 'payment_status_changed:' || coalesce(new.status, 'null'));
  end if;
...
BEGIN
  ...
  SELECT lot_id INTO v_lot FROM public.referral_rewards WHERE id = p_reward_id;
...
-- ===========================================================================
-- Refundace v2 + reverze odměny za doporučení
...
begin
  ...
  if not v_already then
    ...
    -- FÁZE 5: odměna za doporučení se vrací poměrně ke skutečně refundovaným Kč.
    v_referral := public._referral_reverse_for_payment(
      p_payment_id, v_refund_czk / v_payment.paid_amount_czk, 'stripe_refund');
  end if;
...
begin
  ...
  -- 2) FÁZE 5: obnova odměny za doporučení přesně v rozsahu provedené reverze
  --    (do téže sady doporučujícího; nikdy nepřipsané odměny jen stavem).
  v_referral := public._referral_restore_for_payment(p_payment_id);
...
revoke all on function public._referral_reverse_for_payment(uuid, numeric, text) from public, anon, authenticated;
revoke all on function public._referral_restore_for_payment(uuid) from public, anon, authenticated;
