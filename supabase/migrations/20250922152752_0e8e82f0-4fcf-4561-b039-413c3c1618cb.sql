-- Fix setup_crud_test_data to avoid HTTP triggers during testing
...
BEGIN
    ...
    -- Disable triggers temporarily to avoid HTTP calls during testing
    SET session_replication_role = replica;

    -- Create test notification (without triggering HTTP calls)
    ...
    -- Create test payment (without triggering HTTP calls)
    ...
    -- Create test ticket if contest exists (without triggering HTTP calls)
    ...
    -- Create test voucher (without triggering HTTP calls)
    ...
    -- Log the test setup (this should not trigger HTTP calls)
...
EXCEPTION
    WHEN OTHERS THEN
        -- Make sure to reset triggers in case of error
        SET session_replication_role = DEFAULT;
