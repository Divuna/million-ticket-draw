-- Fix: ensure admin_update_affiliate_partner_status termination timestamp
-- always satisfies affiliate_partners contract date CHECK constraints.
--
-- Scope:
--   - Replaces admin_update_affiliate_partner_status only.
--   - Does NOT change transition rules.
--   - Does NOT change commission rates, affiliate codes, registration,
--     partner/register, payments, wallet, buy_ticket_atomic, Partner Offers,
--     customer referrals, B2B partner program, or existing influencer system.
...
BEGIN
  ...
  SELECT status, contract_starts_at, contract_ends_at
  INTO v_old_status, v_contract_starts_at, v_old_contract_ends_at
  ...
  IF v_new_status = 'terminated' THEN
    v_termination_time := clock_timestamp();
    v_new_contract_ends_at := CASE
      WHEN v_contract_starts_at IS NOT NULL
        AND v_termination_time <= v_contract_starts_at
        THEN v_contract_starts_at + interval '1 millisecond'
      ELSE v_termination_time
    END;
  ELSE
    v_new_contract_ends_at := v_old_contract_ends_at;
  END IF;
