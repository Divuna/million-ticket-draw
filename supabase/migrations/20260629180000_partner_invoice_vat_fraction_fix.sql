-- ====================================================================
-- Partner invoice VAT calculation fix (29. 06. 2026)
--
-- Root cause:
--   partners.vat_rate is stored as a FRACTION (default 0.2100, all partners
--   0.21 / 0.00). Two invoice-creation functions divided vat_rate by 100,
--   producing VAT 100x too small for the fraction convention:
--     * create_partner_invoices_for_last_week  -> v_amount_net * (vat_rate/100)
--     * generate_partner_invoice               -> v_amount_net * (vat_rate/100)
--   The bulk function create_partner_invoices_for_period already computes the
--   correct fraction-based VAT (v_coins * price_per_coin * vat_rate) and is
--   left unchanged.
--
-- Fix:
--   Unify on the fraction convention. VAT = net * vat_rate, gross = net + VAT.
--   For net 14.00 and vat_rate 0.21 -> VAT 2.94, gross 16.94.
--
-- Only the two VAT lines change; all other logic is preserved verbatim from
-- 20260612125606_partner_invoice_line_snapshots.sql.
--
-- STAGING FIRST. Production rollout requires separate explicit approval.
-- ====================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: vat_rate is a fraction (0.21), not a percent -> no /100
...
BEGIN
  ...
  -- FIX: vat_rate is a fraction (0.21), not a percent -> no /100
  v_vat_amount := v_amount_net * v_vat_rate;
  v_amount_gross := v_amount_net + v_vat_amount;
