-- Fix: when the last ticket of a contest also hits a bonus position,
-- won_type returned 'bonus' instead of 'main' because the CASE evaluated
-- v_bonus_prize_id IS NOT NULL first.
-- Main win must always take priority: a user who buys the final ticket
-- is the grand-prize winner regardless of any bonus position coincidence.
--
-- Change: swap CASE branch order in the v_result build so 'main' is checked
-- before 'bonus'. No other logic is changed.
