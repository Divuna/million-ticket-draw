-- Homepage "Poslední výherci" needs presentation data only. Keep the existing
-- public winner feed unchanged for other callers and expose a narrower
-- Homepage-specific contract with no ticket join or ticket-number column.

CREATE OR REPLACE FUNCTION public.get_latest_winners_homepage_public(
  winners_limit integer DEFAULT 50
)
RETURNS TABLE(
  public_id text,
  type text,
...
REVOKE ALL ON FUNCTION public.get_latest_winners_homepage_public(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_latest_winners_homepage_public(integer) TO anon;
GRANT EXECUTE ON FUNCTION public.get_latest_winners_homepage_public(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_latest_winners_homepage_public(integer) TO service_role;
