-- Update create_guardian_message_for_user function to use sender='admin' instead of 'system'
CREATE OR REPLACE FUNCTION public.create_guardian_message_for_user(
  p_contest_id uuid,
  p_prize_title text,
  p_user_id uuid
)
...
BEGIN
  ...
  VALUES (
    p_user_id,
    'admin',  -- Changed from 'system' to 'admin'
    '🎉 Gratulujeme k výhře "' || p_prize_title ||
    '". Pro převzetí této výhry je nutný zákonný zástupce. Prosím kontaktujte nás přes chat.',
    now()
  );
END;
$$;

-- Update trigger_guardian_message_on_winner to use sender='admin' instead of 'system'
...
BEGIN
  ...
  IF v_user_age < 18 THEN
    -- Check if message already exists to prevent duplicates (now checks sender='admin')
    IF NOT EXISTS (
...
-- Optionally update existing system messages to admin (one-time migration)
UPDATE messages SET sender = 'admin' WHERE sender = 'system';;
