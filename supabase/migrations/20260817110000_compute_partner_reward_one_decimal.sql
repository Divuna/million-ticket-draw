-- MioCoin — one-decimal rule, part 2/3: the reward engine.
--
-- Bug this fixes (verified read-only on production xkzhjldrojjlrkezorey, 16. 08. 2026):
--   compute_partner_reward returned `floor(v_total_mc)::integer`. BOHEMIA INFINITY
--   runs reward_mode = 'selected_products' with a per-SKU fixed_mc rule below 2 MC,
--   so the engine computed the correct fractional amount and then floored it away.
--   Consequence: the product badge showed nothing, the cart summary showed nothing,
--   and no reward code could ever be created for a sub-1 MC product.
--
-- What changes:
