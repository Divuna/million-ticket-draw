-- Update unlock_ticket function to properly handle bonus win detection
...
DECLARE
    ...
    v_existing_winner winners%rowtype;
    v_won_prize text := null;
    v_won_type text := null;
    v_bonus_prize_id uuid := null;
BEGIN
    -- Get contest details
    ...
    -- Check for ANY bonus prize at this position (regardless of status)
    ...
    IF FOUND THEN
        v_bonus_prize_id := v_bonus_prize.id;
        
        IF v_bonus_prize.status = 'pending' THEN
            -- Fresh bonus win - process normally
        ...
        ELSIF v_bonus_prize.status = 'won' THEN
            -- Check if current user is the winner of this bonus
            SELECT * INTO v_existing_winner
            FROM winners w
            WHERE w.contest_id = unlock_ticket.contest_id
              AND w.prize_id = v_bonus_prize.id
              AND w.user_id = unlock_ticket.user_id
              AND w.type = 'bonus';
              
            IF FOUND THEN
                -- Current user already won this bonus
                v_won_prize := v_bonus_prize.description;
                v_won_type := 'bonus';
            END IF;
        END IF;
    ...
    -- Find next bonus position (only pending ones)
    ...
    -- Return result as JSON with enhanced win detection fields
