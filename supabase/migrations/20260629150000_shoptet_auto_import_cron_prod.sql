-- Automatic Shoptet import scheduler (PRODUCTION xkzhjldrojjlrkezorey).
--
-- Production counterpart of 20260629140000_shoptet_auto_import_cron.sql (staging).
-- Identical logic; the ONLY difference is v_url, which points at the production
-- functions base URL, and the Vault secret description ('(production)').
--
...
--
-- Scope: BOHEMIA config (shoptet_customer_delivery = 'partner'), the Partner API
-- (partner-activate), and the partner dashboard are NOT modified.
--
-- Rollback:
...
begin
  if not exists (select 1 from vault.secrets where name = 'shoptet_cron_internal_token') then
    perform vault.create_secret(
      ...
      'Internal dispatch token for pg_cron -> import-shoptet-orders (production)'
    );
  end if;
end $$;
...
as $$
declare
  -- PRODUCTION functions base URL.
  v_url     text := 'https://xkzhjldrojjlrkezorey.supabase.co/functions/v1/import-shoptet-orders';
  v_token   text;
  v_partner record;
  v_dispatched      int := 0;
