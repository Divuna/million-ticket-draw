-- Run the whole file in Supabase SQL Editor (Dashboard → SQL).
--
-- Before: optional check —
--   SELECT p.proname FROM pg_proc p
--   JOIN pg_namespace ns ON ns.oid = p.pronamespace
--   WHERE ns.nspname = 'public' AND p.proname = 'buy_ticket_atomic';
--
-- After: run supabase/sql/verify_buy_ticket_atomic.sql for pg_get_functiondef + NOTIFY below reloads PostgREST.

DROP FUNCTION IF EXISTS public.buy_ticket_atomic(uuid, uuid);
...
-- Refresh PostgREST schema cache so /rest/v1/rpc/buy_ticket_atomic resolves immediately
