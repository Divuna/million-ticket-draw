  ...
  try {
    ...
    const [{ data: lead }, { data: incoming }] = await Promise.all([
      ...
      admin.from("sales_lead_activities")
        .select("id,lead_id,activity_type,metadata,rfc_message_id")
        .eq("id", activityId)
        .eq("lead_id", leadId)
    ...
    const { data: suppressed } = await admin.from("sales_lead_email_suppression")
      .select("id")
    ...
    const threadHeaders = buildReplyHeaders(parentMessageId, referencesFromMetadata(incoming.metadata));
    ...
    const emailPayload: Record<string, unknown> = {
      ...
      subject,
      text: renderedBody,
      html: `<div style="white-space:pre-wrap;font-family:Arial,sans-serif;font-size:14px;line-height:1.5">${escapeHtml(renderedBody)}</div>`,
      headers: threadHeaders,
    };
    if (attachmentResult.attachments.length > 0) emailPayload.attachments = attachmentResult.attachments;

    const response = await new Resend(key).emails.send(emailPayload as never);
    ...
    const { error: historyError } = await admin.from("sales_lead_activities").insert({
      ...
      metadata: {
        ...
        to: recipient,
        reply_to_activity_id: activityId,
    ...
    if (historyError) {
      return json({ success: true, lead_id: leadId, sent_to: recipient, history_recorded: false, warning: "history_write_failed_after_send" });
    }
