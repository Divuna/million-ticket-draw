...
- **⚠️ Pre-existující produkční defekt (mimo PR, NEOPRAVENO):** produkční trigger funkce `reverse_referral_reward_on_payment_status_change()` má stejné nejednoznačné poziční volání, takže **každá změna stavu platby z `completed` u platby s odměnou `earned` skončí chybou `42725` a vrátí se zpět**. V produkci 16 odměn `earned`, 0 `reversed` — reverzní větev nikdy neproběhla. Dokud se to neopraví, `prepare_stripe_refund` u takové platby vyhodí výjimku. Oprava je jednořádková (stejná pojmenovaná notace) a čeká na samostatné schválení.
