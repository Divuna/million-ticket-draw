...
- **Refund after the commission is locked (confirmed by Pavel, 2026-09-24):** „Affiliate provize, kterou už kvůli vystavenému dokladu / dávce / výplatě nelze zpětně snížit, se při následné refundaci eviduje jako recovery a automaticky se umořuje z budoucích provizí stejného affiliate. Jiné provize se nemění a nevzniká záporná provize.“ The amount is never collected manually. The recovery equals the commission on the refunded CZK (cumulative: 300 CZK → 15 CZK, refund 100 CZK → recovery 5 CZK, another 50 CZK → recovery 7.50 CZK in total). The oldest open recovery is repaid first; only the remainder of a new commission is paid out (example: recovery 25 CZK → new commission 15 CZK → payout 0, recovery 10 CZK → next commission 20 CZK → 10 CZK repays the recovery, payout 10 CZK). If the Stripe refund finally fails, exactly the recovery created by that refund is cancelled and any part already repaid from a later commission is returned to the affiliate as a payable claim.
...
- **Accounting / tax:** the recovery and its repayment are only recorded technically. No corrective tax document (self-billed invoice correction) is issued automatically and issued documents are never changed. How the recovery and the VAT part for VAT-payer affiliates are handled in accounting needs a separate accounting/legal decision.
- **Recovery without future commissions:** if the affiliate has no further customer commission, the recovery stays open; likewise a returned claim after a failed refund waits for the next customer commission of that affiliate. Paying such a claim out separately, or repaying a recovery from a company commission, would be a new decision.
- **Company commission after invoice correction:** the invoice status `void` exists, but nothing moves a paid invoice to `void` and there is no credit note; a company commission therefore never reacts to a later correction or cancellation of its source invoice. Needs a separate decision if such corrections are to be supported.
